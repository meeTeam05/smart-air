...
{
    ESP_LOGI(TAG,
             "Initializing I2C bus on port %d (SDA: GPIO%d, SCL: GPIO%d, Speed: %lu Hz)",
             port,
             sda_gpio,
             scl_gpio,
             clk_speed);
...
esp_err_t i2c_dev_init(i2c_dev_t *dev)
{
    ...
    esp_err_t ret = i2c_master_bus_add_device(i2c_bus_handle, &dev_cfg, (i2c_master_dev_handle_t *)&dev->dev_handle);
    if (ret != ESP_OK) {
...
esp_err_t i2c_dev_read_reg(i2c_dev_t *dev, uint8_t reg, void *data, size_t len)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C read failed from addr 0x%02x reg 0x%02x: %s", dev->addr, reg, esp_err_to_name(ret));
    }
...
esp_err_t i2c_dev_write_reg(i2c_dev_t *dev, uint8_t reg, const void *data, size_t len)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C write failed to addr 0x%02x reg 0x%02x: %s", dev->addr, reg, esp_err_to_name(ret));
    }
#if CONFIG_I2CDEV_DEBUG
    else {
