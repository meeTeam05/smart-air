...
typedef struct { float ratio; float ppm; } ratio_point_t;
...
{
    if (fabsf(x2 - x1) < 1e-6f) return y1;
    float t = (x - x1) / (x2 - x1);
    return y1 + t * (y2 - y1);
}
...
static float ratio_to_ppm_no2(float ratio)
{
    ...
    for (int i = 0; i < NO2_CURVE_LEN - 1; i++) {
        if (ratio >= NO2_CURVE[i].ratio && ratio <= NO2_CURVE[i + 1].ratio) {
            return lerp(
                NO2_CURVE[i].ratio, NO2_CURVE[i].ppm,
                NO2_CURVE[i + 1].ratio, NO2_CURVE[i + 1].ppm,
                ratio
            );
        }
    }
...
{
    if (vout_v < 0.001f) return rl * 1000.0f;  /* Avoid division by zero */
...
esp_err_t gm102b_init(gm102b_t *dev, adc_channel_t channel, float rl, float vc)
{
    ...
    ESP_LOGI(TAG, "NO2 sensor initialized on ADC1 channel %d (RL=%.0f ohm, VC=%.1fV)",
             (int)channel, rl, vc);
    return ESP_OK;
}
