...
class _WifiSetupScreenState extends ConsumerState<WifiSetupScreen> {
  ...
  Future<void> _provision() async {
    ...
    try {
      // Step 1 - BLE: send credentials and wait for the device to connect WiFi.
      ...
      final provision = await _bleService.sendCredentials(
        _ssidCtrl.text,
        _passCtrl.text,
      );
      ...
      await ref
          .read(deviceServiceProvider)
          .configureProvisionedDevice(
            host: provision.ip,
            deviceId: provision.deviceId,
            secretKey: registration.secretKey,
          );

      // Step 2 - The device is on WiFi; poll until MQTT status arrives.
